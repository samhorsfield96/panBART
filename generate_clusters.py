...
def query_model(rank, model_path, world_size, args, BARTlongformer_config, tokenizer, prompt_list, genome_labels, cluster_assignments, DDP_active, encoder_only):
    if DDP_active:
    ...
    for n_neighbors in n_neighbors_list:
        for leiden_resolution in leiden_resolution_list:
            ...
            sc.pp.neighbors(
                adata,
                n_neighbors=n_neighbors,   # try 10–50
                metric="cosine"
            )
            ...
            df_pred['Taxon'] = query_list_df.iloc[:, 0].values
            df_pred['predicted_label'] = leiden_labels
            df_pred.to_csv(args.outpref + f"_n_neighbors_{n_neighbors}_leiden_resolution_{leiden_resolution}_predictions.tsv", sep='\t', index=False)
...
def main():
    ...
    if args.prompt_labels != None:
        ...
        prompt_list, genome_labels = read_prompt_file(args.query_file)
    ...
    if DDP_active:
        ...
        with Manager() as manager:
            ...
            mp.spawn(query_model,
                    args=(args.model_path, world_size, args, BARTlongformer_config, tokenizer, prompt_list, genome_labels, cluster_assignments, DDP_active, args.encoder_only),
                    nprocs=world_size,
                    join=True)
            return_list = list(mp_list)
    else:
        query_model(device, args.model_path, 1, args, BARTlongformer_config, tokenizer, prompt_list, genome_labels, cluster_assignments, DDP_active, args.encoder_only)
