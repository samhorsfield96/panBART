...
def parse_args():
    ...
    parser.add_argument("--gene_list", type=str, required=True, help="Path to the text file containing a list of representative gene IDs to calculate probabilities for.")
...
def calculate_pseudolikelihood(model, tokenizer, loader, device, max_seq_length, encoder_only, token_query_list):
    model.eval()
    mask_token = tokenizer.encode("<mask>").ids[0]
    pad_token = tokenizer.encode("<pad>").ids[0]

    total_gene_list = []
    average_gene_dict = defaultdict(float)
    with torch.no_grad():
        ...
        for decoder_input, encoder_input, labels, decoder_attention_mask, encoder_attention_mask, global_attention_mask in loader:  # Correctly unpack the tuples returned by the DataLoader
            ...
            for i in range(0, total_len, max_seq_length):
                ...
                if encoder_only:
                    ...
                    for j in tqdm(range(token_count)):
                        ...
                        for token_id in token_query_list:
                            log_pseudo_likelihood_gene = get_pseudolikelihood(outputs, j, token_id, batch_encoder_input)
                            
                            genome_gene_dict[token_id].append(log_pseudo_likelihood_gene)
            ...
            for token_id in token_query_list:
                average_likelihood_gene = mean(genome_gene_dict[token_id])
                average_gene_dict[token_id].append(average_likelihood_gene)
...
def main():
    ...
    gene_query_list = []
    token_query_list = []
    with open(args.token_list, "w") as f:
        for line in f:
            split_line = line.rstrip()
            try:
                token = tokenizer.encode(str(reps_dict[split_line])).ids[0]
                token_query_list.append(token)
                gene_query_list.append(split_line)
            except KeyError:
                print(f"{split_line} not in reps_dict")
 ...
 return_list = []
    ...
    for entry in gene_list:
        for key, genome_list in entry.items():
    ...
    with open(args.outpref + "_genome_pseudolikelihood.txt", "w") as f:
        f.write("Genome_ID\t" + "\t".join(gene_query_list) + "\n")
    ...
    with open(args.outpref + "_locus_pseudolikelihood.txt", "w") as f:
        f.write("Genome_Index\tGene_name\t" + "\t".join([str(x) for x in range(0, len(return_list))]) + "\n")
        for genome_idx, likelihood_dict in enumerate(return_list):
            gene_idx = 0
            for token, likelihood_list in likelihood_dict.items():
                
                # add zeros to fill list up
                while len(likelihood_list) < len(return_list):
                    likelihood_list.append(0.0)

                gene_name = gene_query_list[gene_idx]
                f.write(str(genome_idx) + "\t" + gene_name + "\t".join([str(x) for x in likelihood_list]) + "\n")
                gene_idx += 1
